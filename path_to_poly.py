	...
	def path_to_polygon(path, distance_threshold):
		...
		for node in path:
			...
			while not closed:
				...
				while max_node_distance < (distance_threshold**2):
				...
				#once we exit from the while loop (sqrt(max_node_distance) has excedded the threshold), remove all nodes between P1 and P2
				while midP != P2:
