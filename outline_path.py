...
class OutlinePath:
	...
	def outline_outer_image(img, path_num = -1, min_path_length = 0):
		...
		if (current_vertex is not None and path_num != 0):
			...
			if path_num > 0:
				return_list += OutlinePath.outline_outer_image(inverted_image, path_num = path_num, min_path_length = min_path_length)
			elif path_num < 0:
				return_list += OutlinePath.outline_outer_image(inverted_image, min_path_length = min_path_length)
...
if __name__ == "__main__":
	...
	img_array = OutlinePath.img_to_bool_array(image, 200)
	plt.subplot(1,2,2)
	plt.imshow(img_array, 'gray')


	paths = OutlinePath.outline_outer_image(img_array, min_path_length = 10)
	paths = OutlinePath.cull_paths(paths, 6)
